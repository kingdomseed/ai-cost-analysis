...
import cursorPricing from "../../../../data/cursor-pricing.2026-03-11.json";
...
function calculateBlendedRate(model: Model, ratio: number): number {
  const weightInput = ratio / (ratio + 1);
  const weightOutput = 1 / (ratio + 1);
  return model.rates.input * weightInput + model.rates.output * weightOutput;
}

function calculateBlendedRateWithCaching(
  model: Model,
  ratio: number,
  ...
  if (useCaching && model.rates.cache_read && model.rates.cache_write && cacheHitRate > 0) {
    ...
    // Cost per 1M input tokens with caching:
    // - Cached portion: write once + read 3x at cheaper rate
    // - Uncached portion: input rate 3x
    ...
    // Average across all re-reads to get per-request rate
    effectiveInputRate = (cacheWriteCost + cacheReadCost + uncachedCost) / RE_READS;
  } else {
    effectiveInputRate = model.rates.input;
  }
  
  return effectiveInputRate * weightInput + model.rates.output * weightOutput;
}
...
function ModelBadge({ model, onRemove }: { model: Model; onRemove?: () => void }) {
  const color = PROVIDER_COLORS[model.provider] || "bg-gray-400";
  return (
    <span 
      onClick={onRemove}
      className="inline-flex items-center gap-1.5 bg-white border border-[#e0e0d8] rounded-full px-3 py-1.5 text-sm cursor-pointer hover:border-[#14120b]/30 transition-colors"
    >
      <span className={`w-2 h-2 rounded-full ${color}`} />
      <span className="font-medium">{model.name}</span>
      {onRemove && (
        <span className="ml-1 text-[#14120b]/40 hover:text-red-500">×</span>
      )}
    </span>
  );
}
...
export default function CursorCalculator() {
  ...
  const selectedModelObjects = useMemo(() => apiModels.filter((m) => selectedModels.includes(m.id)), [selectedModels]);
    ...
    <div className="min-h-screen bg-[#f7f7f4] text-[#14120b]">
      ...
      <header className="bg-white border-b border-[#e0e0d8]">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <a href="/" className="text-sm text-[#14120b]/50 hover:text-[#14120b]">Main →</a>
        </div>
      </header>
